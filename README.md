...
| Linux / POSIX           | Windows Desktop                       | Supported here           | Uses the provisioned Zig cross toolchain to build Windows templates from a Linux host.                                              |
...
- `--platform TUPLE` — target platform tuple for the build. Supported tuples are `windows/amd64` on Windows hosts, `linux/amd64` on Linux/POSIX hosts, and `windows/amd64` on Linux/POSIX hosts via the Zig cross-compilation flow. Defaults to host if not supplied.
