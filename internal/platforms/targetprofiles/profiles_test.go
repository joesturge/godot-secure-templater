...
func TestSConsTargetProfileTemplateNames(t *testing.T) {
	...
	assert.Contains(t, profile.ExtraSConsArgs, "use_llvm=yes", "Windows profile should force LLVM mode so SCons uses bundled Zig directly")
	assert.Contains(t, profile.ExtraSConsArgs, "use_mingw=no", "Windows profile should avoid MinGW mode and keep the build Zig-only")
