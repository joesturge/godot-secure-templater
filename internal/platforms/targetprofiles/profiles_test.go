...
func TestSConsTargetProfileTemplateNames(t *testing.T) {
	...
	assert.Contains(t, profile.ExtraSConsArgs, "use_llvm=yes", "Windows profile should force LLVM mode for the Zig compiler path")
	assert.NotContains(t, profile.ExtraSConsArgs, "use_mingw=yes", "Windows profile should not require MinGW when using the provisioned Zig compiler")
